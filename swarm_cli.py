...
def get_task(
    ...
    try:
        ...
        if result is not None:
            ...
            ):  # Catch cases where result was a non-JSON string or other non-None, non-dict/list type
                console.print(
        ...
        if e.response.status_code == 404:
            ...
            typer.secho(
                f"Error fetching task {task_id}: {e}. Response: {e.response.text}",
                fg=typer.colors.RED,
            )
    except requests.exceptions.RequestException as e:
        typer.secho(f"Request failed: {e}", fg=typer.colors.RED)
...
@app.command()
def get_summary(
    task_id: str = typer.Argument(..., help="The ID of the task to get a summary for.")
):
    ...
    try:
        ...
        if e.response.status_code == 404:
            ...
            typer.secho(
                f"Error fetching task summary for {task_id}: {e}. Response: {e.response.text}",
                fg=typer.colors.RED,
            )
    except requests.exceptions.RequestException as e:
        typer.secho(f"Request failed: {e}", fg=typer.colors.RED)
