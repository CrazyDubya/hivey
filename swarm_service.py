...
# Standard library and FastAPI imports
import sqlite3
import logging
...
if not EXPECTED_API_KEY:
    ...
    logger.info(f"SWARM_API_KEY successfully loaded. Length: {len(EXPECTED_API_KEY)}")
...
def periodic_supervisor_check_loop():
    ...
    while not shutdown_event.is_set():
        try:
            ...
            logger.debug(
                f"Supervisor loop: Calling process_pending_subtasks() at {datetime.now()}."
            )
            process_pending_subtasks()

            # Then check for stalled subtasks
            logger.debug(
                f"Supervisor loop: Calling check_for_stalled_subtasks() at {datetime.now()}."
            )
            check_for_stalled_subtasks()

            # Then check if any parent tasks are complete
            logger.debug(
                f"Supervisor loop: Calling check_and_collate_supervised_tasks() at {datetime.now()}."
            )
...
def create_task_record(
    ...
    try:
        logger.info(
            f"Attempting to insert task {task_id} into database. Parent: {parent_task_id}, Is Subtask: {is_subtask}"
        )
        db_cursor.execute(
        ...
        logger.info(
            f"Task {task_id} insert executed. Cursor rowcount: {db_cursor.rowcount if db_cursor else 'N/A'}"
        )
...
def get_task_status_summary(task_id: str) -> Optional[Dict[str, Any]]:
    ...
    if not cursor:
        logger.error(
            f"Failed to get DB cursor for get_task_status_summary(task_id={task_id})"
        )
        return None
    try:
        cursor.execute(
...
def run_swarm_task(task_id: str, description: str) -> None:
    ...
    logger.info(
        f"[{task_id}] Background processing initiated for: {description[:50]}..."
    )
    ...
    try:
        ...
        if not task_details:
            ...
            update_task_status(
                task_id, "failed", error_message="Could not retrieve task details."
            )
            return
        ...
        if is_subtask:
            logger.info(
                f"[{task_id}] Is a subtask. Assigned to: {assigned_agent_name}. Description: {current_task_description[:50]}..."
            )
            ...
            logger.info(
                f"[{task_id}] Subtask execution handled by swarm_instance.execute_subtask."
            )

        else:  # Parent task or simple task (not a subtask)
            logger.info(
                f"[{task_id}] Is a parent or simple task. Description: {current_task_description[:50]}..."
            )
            ...
            if task_output_dict and "subtask_ids" in task_output_dict:
                logger.info(
                    f"[{task_id}] Task decomposed into subtasks: {task_output_dict.get('subtask_ids')}. Parent status set to 'awaiting_subtasks' by Swarm.run."
                )
                ...
                logger.error(
                    f"[{task_id}] Error returned by swarm_instance.run: {task_output_dict['error']}"
                )
                update_task_status(
                    task_id,
                    "failed",
                ...
                logger.info(
                    f"[{task_id}] Task processed directly by Swarm.run (no decomposition). Output: {str(task_output_dict)[:100]}..."
                )
                update_task_status(
                    task_id,
                    "completed",
                ...
                logger.warning(
                    f"[{task_id}] swarm_instance.run returned an unexpected or None value. Marking as failed."
                )
                update_task_status(
                    task_id,
                    "failed",
        ...
        logger.error(
            f"[{task_id}] Unexpected error in run_swarm_task: {e}", exc_info=True
        )
        update_task_status(
            task_id,
            "failed",
...
def process_pending_subtasks():
    ...
    try:
        ...
        logger.error(
            f"Supervisor: DB error fetching pending subtasks: {e}", exc_info=True
        )
        return
    ...
    logger.info(
        f"Supervisor: Found {len(pending_subtasks)} pending subtasks to process."
    )
    for subtask_row in pending_subtasks:
        subtask_id, subtask_description, assigned_agent_name = subtask_row
        logger.info(
            f"Supervisor: Processing pending subtask {subtask_id} assigned to {assigned_agent_name}."
        )
        ...
        try:
            ...
            logger.error(
                f"Supervisor: Error starting thread for subtask {subtask_id}: {e}",
                exc_info=True,
            )
            update_task_status(
                subtask_id,
...
def check_for_stalled_subtasks():
    ...
    try:
        ...
        logger.error(
            f"Supervisor: DB error fetching stalled subtasks: {e}", exc_info=True
        )
        return
    ...
    for subtask_row in stalled_subtasks:
        ...
        logger.warning(
            f"Supervisor: Found stalled subtask {subtask_id} assigned to {assigned_agent_name}. Last updated: {updated_at}"
        )
        ...
        logger.info(
            f"Supervisor: Marked stalled subtask {subtask_id} as failed due to timeout."
        )
...
def check_and_collate_supervised_tasks():
    ...
    try:
        db_cursor.execute(
            "SELECT task_id, description FROM tasks WHERE status = 'awaiting_subtasks' AND is_subtask = 0"
        )
    ...
    for parent_task_row in parent_tasks_awaiting:
        ...
        logger.info(
            f"Supervisor: Checking parent task {parent_task_id} ('{parent_task_description[:50]}...')"
        )
        ...
        try:
            ...
            logger.error(
                f"Supervisor: DB error fetching subtasks for {parent_task_id}: {e}",
                exc_info=True,
            )
        ...
        if not subtasks:
            logger.warning(
                f"Supervisor: Parent task {parent_task_id} is 'awaiting_subtasks' but has no subtasks found. Marking as error."
            )
            update_task_status(
                parent_task_id,
                "failed",
        ...
        for sub_row in subtasks:
            ...
            if sub_status not in ("completed", "failed"):
                ...
                logger.debug(
                    f"Supervisor: Parent {parent_task_id} still waiting for subtask {sub_task_id} (status: {sub_status})."
                )
            ...
            if sub_status == "completed":
                try:
                    ...
                    if isinstance(sub_result_json, dict):
                        ...
                        try:
                            sub_result_data = (
                                json.loads(sub_result_json) if sub_result_json else {}
                            )
                        except json.JSONDecodeError:
                            # If it's not valid JSON, just use the string directly
                            logger.warning(
                                f"Supervisor: Could not parse subtask result as JSON: {sub_result_json[:100]}..."
                            )
                    ...
                    logger.error(
                        f"Supervisor: Error decoding result JSON for subtask {sub_task_id}: {e}. Original: {sub_result_json}"
                    )
                    subtask_results_for_collation.append(
                        {
                ...
                failed_subtask_errors.append(
                    f"Subtask {sub_task_id} failed: {sub_error_msg or 'Unknown error'}"
                )

        if all_subtasks_finished:
            logger.info(
                f"Supervisor: All subtasks finished for parent {parent_task_id}."
            )
            if failed_subtask_errors:
                parent_error_message = "One or more subtasks failed: " + "; ".join(
                    failed_subtask_errors
                )
                logger.error(
                    f"Supervisor: Parent task {parent_task_id} failed due to subtask errors: {parent_error_message}"
                )
                update_task_status(
                    parent_task_id, "failed", error_message=parent_error_message
                )
            else:
                try:
                    logger.info(
                        f"Supervisor: Collating {len(subtask_results_for_collation)} results for parent {parent_task_id}."
                    )
                    ...
                    logger.info(
                        f"Supervisor: Parent task {parent_task_id} completed successfully after collation."
                    )
                except Exception as e:
                    collation_error_msg = (
                        f"Error during result collation for {parent_task_id}: {e}"
                    )
                    logger.error(f"Supervisor: {collation_error_msg}", exc_info=True)
                    update_task_status(
                        parent_task_id, "failed", error_message=collation_error_msg
                    )
        else:
            logger.debug(
                f"Supervisor: Parent {parent_task_id} still has pending subtasks."
            )
...
async def submit_task(
    ...
    logger.info(
        f"submit_task entered. Request description (first 50 chars): {request.task_description[:50]}..."
    )
    task_id = str(uuid.uuid4())
    logger.info(
        f"Generated task_id: {task_id} for request: {request.task_description[:50]}..."
    )
    ...
    try:
        ...
        logger.info(
            f"Task record created for task_id: {task_id}. Adding to background tasks."
        )
        ...
        logger.error(
            f"UNEXPECTED ERROR IN SUBMIT_TASK for task_id {task_id if 'task_id' in locals() else 'UNKNOWN'}: {e}",
            exc_info=True,
        )
