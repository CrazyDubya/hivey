...
async def call_ollama_chat_async(
    ...
    try:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=timeout)) as session:
            async with session.post(f"{OLLAMA_BASE_URL}/chat", json=payload) as response:
                response.raise_for_status()
                result = await response.json()
                logger.info(f"Ollama async call successful for model: {model_name}")
                return result
                
    except aiohttp.ClientError as e:
        logger.error(f"Async HTTP error calling Ollama for model {model_name}: {e}")
        return None
    except asyncio.TimeoutError:
        logger.error(f"Async timeout calling Ollama for model {model_name}")
        return None
    except json.JSONDecodeError as e:
        logger.error(f"Async JSON decode error for Ollama model {model_name}: {e}")
        return None
    except Exception as e:
        logger.error(f"Unexpected async error calling Ollama model {model_name}: {e}")
        return None
...
async def call_xai_chat_async(
    ...
    **kwargs
    ...
    try:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=timeout)) as session:
            async with session.post(f"{XAI_API_BASE_URL}/chat/completions", 
                                  json=payload, headers=headers) as response:
                response.raise_for_status()
                result = await response.json()
                logger.info(f"X.AI async call successful for model: {model_name}")
                return result
                
    except aiohttp.ClientError as e:
        logger.error(f"Async HTTP error calling X.AI for model {model_name}: {e}")
        return None
    except asyncio.TimeoutError:
        logger.error(f"Async timeout calling X.AI for model {model_name}")
        return None
    except json.JSONDecodeError as e:
        logger.error(f"Async JSON decode error for X.AI model {model_name}: {e}")
        return None
    except Exception as e:
        logger.error(f"Unexpected async error calling X.AI model {model_name}: {e}")
        return None


async def batch_llm_calls(calls: List[Dict[str, Any]]) -> List[Optional[Dict[str, Any]]]:
    """
    ...
    for call in calls:
        ...
        if provider == "ollama":
            task = call_ollama_chat_async(model_name, messages, options=options)
        elif provider == "xai":
